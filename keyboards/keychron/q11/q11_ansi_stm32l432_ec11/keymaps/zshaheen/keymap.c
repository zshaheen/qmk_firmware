...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_91(
        ...
        TO(MOUSE_KEYS),  KC_LCTL,  KC_LOPT,  KC_LCMD,  MO(MAC_FN),         KC_SPC,                        KC_SPC,             MK_WH_TOGGLE,  KC_RCMD, KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    [WIN_BASE] = LAYOUT_ansi_91(
        KC_MUTE,  KC_ESC,   KC_BRID,  KC_BRIU,  KC_TASK,  KC_FLXP,    RGB_VAD,   RGB_VAI,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,    KC_VOLU,  KC_INS,   KC_DEL,   KC_MUTE,
        TO(MAC_BASE),  KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,      KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,    KC_EQL,   KC_BSPC,            KC_PGUP,
        TO(MAC_FN),  KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,      KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,    KC_RBRC,  KC_BSLS,            KC_PGDN,
        TO(WIN_BASE),  KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,      KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,             KC_HOME,
        TO(WIN_FN),  KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,      KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,  KC_UP,
        TO(MOUSE_KEYS),  KC_LCTL,  KC_LWIN,  KC_LALT,  MO(WIN_FN),         KC_SPC,                        KC_SPC,             MK_WH_TOGGLE,  KC_RALT, KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    [MOUSE_KEYS] = LAYOUT_ansi_91(
        ...
        TO(MOUSE_KEYS),  KC_LCTL,  KC_LOPT,  KC_LCMD,  MO(WIN_FN),            KC_SPC,                       KC_SPC,            MK_WH_TOGGLE, KC_RCMD, KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT),
};
...
void ToggleMKWHLayer(void) {
  if (IS_LAYER_ON(MOUSE_KEYS)) {
    layer_off(MOUSE_KEYS);
  } else if (IS_LAYER_OFF(MOUSE_KEYS)) {
    layer_on(MOUSE_KEYS);
  }
}
      ...
      layer_state_t highest_layer = get_highest_layer(layer_state|default_layer_state);
        switch(highest_layer) {
            case MAC_BASE:
                ...
                break;
            case MAC_FN:
                rgb_matrix_set_color_helper(i, HSV_CORAL, led_min, led_max);
                break;
            case WIN_BASE:
                rgb_matrix_set_color_helper(i, HSV_CYAN, led_min, led_max);
                break;
            case WIN_FN:
                rgb_matrix_set_color_helper(i, HSV_TURQUOISE, led_min, led_max);
                break;
            case MOUSE_KEYS:
